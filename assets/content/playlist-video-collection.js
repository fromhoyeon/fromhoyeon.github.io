/*
  Playlist-driven Video Collection backup.
...
(function initPlaylistVideoCollection(){
  if (window.HOYEON_PLAYLIST_VIDEO_COLLECTION) return;
  ...
  function render(block, work){
    ...
    playerTarget.id = `playlist-video-collection-player-${++instanceCount}`;
    ...
    loadYouTubeApi().then((YT) => {
      ...
      console.warn('[Playlist Video Collection] YouTube playlist unavailable.', error);
      currentTitle.textContent = 'Playlist unavailable';
      status.textContent = '';
      grid.replaceChildren(createMessage('Open playlist ↗', playlistUrl));
    });
  ...
  window.HOYEON_PLAYLIST_VIDEO_COLLECTION = {render, extractPlaylistId};
})();
