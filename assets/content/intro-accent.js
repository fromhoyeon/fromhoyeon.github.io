/*
  Hoyeon intro accent image
  -------------------------
  Intentional identity element shown around the homepage intro, not legacy decoration.
  Image, placement, size and alignment are managed in Sanity Site Copy > Intro.
*/
...
(async function initIntroAccent(){
  ...
  if (!document.querySelector('#intro-accent-styles')) {
    ...
    style.textContent = `
      .intro-accent{width:var(--intro-accent-width,96px);max-width:min(80vw,320px);margin:0;padding:0;border:1px solid var(--media-black)}
